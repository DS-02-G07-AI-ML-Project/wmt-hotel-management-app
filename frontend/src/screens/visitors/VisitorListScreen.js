...
export default function VisitorListScreen({ navigation }) {
  ...
  useListScreenHeader(navigation, { addRoute: 'VisitorForm', addLabel: '＋ Add' });

  const load = useCallback(async () => {
    try {
      setError(null);
      setLoading(true);
      const res = await requestWithFallback('/api/visitors');
    ...
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={styles.card}
            onPress={() => navigation.navigate('VisitorDetail', { id: item._id })}
          >
            <Text style={styles.title}>{item.fullName}</Text>
            <Text style={styles.sub}>
              {item.status} · {item.purpose || 'Visit'}
            </Text>
            <Text style={styles.meta}>Host: {item.hostName || '—'}</Text>
          </TouchableOpacity>
        )}
        ListEmptyComponent={<Text style={styles.empty}>No visitors.</Text>}
      />
    </View>
  );
}
