...
export default function ComplaintDetailScreen({ navigation, route }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await requestWithFallback(`/api/complaints/${id}`);
    ...
    Alert.alert('Delete complaint', 'Are you sure?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          setDeleting(true);
          try {
            const res = await requestWithFallback(`/api/complaints/${id}`, { method: 'DELETE' });
            const json = await res.json();
            if (!res.ok) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.row}>Reported by: {item.reportedBy}</Text>
      <Text style={styles.row}>Room: {item.room?.roomNumber || '—'}</Text>
      <Text style={styles.body}>{item.description}</Text>

      <TouchableOpacity
        style={styles.edit}
        onPress={() => navigation.navigate('ComplaintForm', { id: item._id })}
      >
